...
func dirReduc(_ arr: [String]) -> [String] {
    var resultNavigation = arr.joined(separator: " ")
    let oppositeDirections = ["NORTH SOUTH", "SOUTH NORTH", "EAST WEST", "WEST EAST"]
    var stop = arr.count
    repeat {
      for v in oppositeDirections {
        if resultNavigation.contains(v) {
            resultNavigation = resultNavigation.replacingOccurrences(of: v, with: "")
												.trimmingCharacters(in: .whitespaces).components(separatedBy: " ")
												.filter { !$0.isEmpty }
												.joined(separator: " ")
        }
        stop -= 1
    ...
    return resultNavigation.components(separatedBy: " ").filter { !$0.isEmpty }
}